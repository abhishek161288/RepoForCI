...
open class SwiftyCodeView: UIControl {
    ...
    @IBInspectable open var textLength: Int = 1 {
        didSet {
            setupUI()
        }
    }
...
extension SwiftyCodeView: UITextFieldDelegate, SwiftyCodeTextFieldDelegate {
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if !textField.hasText || (textField.text?.count ?? 0 < textLength) {
            let index = textField.tag
            let item = stackView.arrangedSubviews[index] as! SwiftyCodeItemView
            item.textField.text = textField.text! + string
            sendActions(for: .valueChanged)
            if index == length - 1 && item.textField.text?.count == (textLength) { //is last textfield
                delegate?.codeView(sender: self, didFinishInput: self.code)
                textField.resignFirstResponder()
                return false
            }
            delegate?.codeView(sender: self, didChangeInput: self.code)
            if item.textField.text?.count == (textLength) {
    ...
    public func deleteBackward(sender: SwiftyCodeTextField) {
        for i in 1..<length{
            ...
            if ((itemView.textField.text ?? "").count == 0) {
